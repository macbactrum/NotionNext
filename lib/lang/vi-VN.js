export default {
  LOCALE: 'vi-VN',
  MENU: {
    WALK_AROUND: 'Khám Phá',
    CATEGORY: 'Danh Mục',
    TAGS: 'Thẻ',
    COPY_URL: 'Sao Chép URL',
    DARK_MODE: 'Chế Độ Tối',
    LIGHT_MODE: 'Chế Độ Sáng',
    THEME_SWITCH: 'Đổi Giao Diện'
  },
  NAV: {
    INDEX: 'Trang Chủ',
    RSS: 'RSS',
    SEARCH: 'Tìm Kiếm',
    ABOUT: 'Giới Thiệu',
    MAIL: 'Email',
    ARCHIVE: 'Lưu Trữ'
  },
  COMMON: {
    MORE: 'Xem Thêm',
    NO_MORE: 'Hết Rồi',
    LATEST_POSTS: 'Bài Viết Mới Nhất',
    TAGS: 'Thẻ',
    NO_TAG: 'Không Có Thẻ',
    CATEGORY: 'Danh Mục',
    SHARE: 'Chia Sẻ',
    SCAN_QR_CODE: 'Quét Mã QR',
    URL_COPIED: 'Đã sao chép URL!',
    TABLE_OF_CONTENTS: 'Mục Lục',
    RELATE_POSTS: 'Bài Viết Liên Quan',
    COPYRIGHT: 'Bản Quyền',
    AUTHOR: 'Tác Giả',
    URL: 'URL',
    POSTS: 'Bài Viết',
    ARTICLE: 'Bài Viết',
    VISITORS: 'Lượt Truy Cập',
    VIEWS: 'Lượt Xem',
    COPYRIGHT_NOTICE: 'Tất cả bài viết trong blog này, trừ khi có ghi chú đặc biệt, đều áp dụng thỏa thuận BY-NC-SA. Vui lòng ghi rõ nguồn!',
    RESULT_OF_SEARCH: 'Kết Quả Tìm Kiếm',
    ARTICLE_DETAIL: 'Chi Tiết Bài Viết',
    PASSWORD_ERROR: 'Mật Khẩu Sai!',
    ARTICLE_LOCK_TIPS: 'Vui lòng nhập mật khẩu:',
    SUBMIT: 'Gửi',
    POST_TIME: 'Đăng vào',
    LAST_EDITED_TIME: 'Chỉnh sửa lần cuối',
    COMMENTS: 'Bình Luận',
    RECENT_COMMENTS: 'Bình Luận Gần Đây',
    DEBUG_OPEN: 'Gỡ Lỗi',
    DEBUG_CLOSE: 'Đóng',
    THEME_SWITCH: 'Đổi Giao Diện',
    ANNOUNCEMENT: 'Thông Báo',
    START_READING: 'Bắt Đầu Đọc',
    MINUTE: 'phút',
    WORD_COUNT: 'Số Từ'
  },
  PAGINATION: {
    PREV: 'Trước',
    NEXT: 'Tiếp'
  },
  SEARCH: {
    ARTICLES: 'Tìm Kiếm Bài Viết',
    TAGS: 'Tìm Kiếm Trong'
  },
  POST: {
    BACK: 'Quay Lại',
    TOP: 'Lên Đầu'
  }
}
